...
let allNews = originalObj.map(item => {
  return { ...item, date2: formatDate(item.date) };
});
...
let displayNews = filteredNews => {
  newsDiv.innerHTML = `${filteredNews
    .map(
      materia => `
		<div class="news-style"> 
			<h2 class="news-style title-style">${materia["title"]} </h2>
			<h3 class="time-style">${formatDate(materia["date"])} </h3>
  ...
  totalOfNews.innerHTML = `<p>Localizamos ${sumFilteredNews} notícia(s)</p>`;
};

let populateDropListDates = arr => {
  let filteredNewsDate = arr.filter(materia => {
    return materia.date2;
  });
...
function loadByDate() {
  ...
  let filteredNews = loadByChannel().filter(materia => {
